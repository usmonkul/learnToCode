...
- **Dark-mode pairing on every new `bg-brand-*`/`bg-brand2-*` light-chip or colored-section usage — added during Task 4's review, binding on every task from here on** (this plan's own code blocks for Tasks 4+ do not reliably include these; add them by hand, following AGENTS.md's documented pattern, which names exactly this case — a `bg-brand-50`/`bg-brand-100`-style light chip — as needing a hand-added `dark:` variant): a small chip/badge/circle (`bg-brand-100/200/300` + a dark `text-brand-700/800/900`) gets `dark:bg-brand-950 dark:text-brand-400` (or the `brand2` equivalent). A large colored section background (`bg-brand-100/200`) gets `dark:bg-brand-900` or `dark:bg-brand2-950` depending on how much body text sits directly on it (prefer `-950` when the only text on it is a small label already getting its own `dark:` pairing; prefer `-900` when larger body copy/buttons sit on it, so the section doesn't go near-black). A standalone kicker/label (`text-brand-700`, no background of its own, sitting on the page's `bg-canvas`/`bg-canvas-muted`) gets `dark:text-brand-300`. Body text sitting inside a section whose background just got a `dark:bg-brand-900` pairing (e.g. `text-brand-900` on `bg-brand-200`) needs its own `dark:text-brand-100` (or similar light step) so it doesn't go dark-on-dark. A bare icon with no background (`text-brand-600`, no `bg-`) does **not** need a `dark:` variant — this matches the already-reviewed, already-shipped pattern in `TopNav.jsx`/`UserMenu.jsx` (Task 2), where `text-brand-600` icons read fine unpaired in both themes since the color itself is mid-lightness. When in doubt, check: does this element's *background* change lightness dramatically between light/dark mode while its *text* color stays fixed? If yes, it needs a pairing; if it's just a mid-tone accent on the page's own (already-adapting) background, it doesn't.
